...
const LiveWebinarView = ({
  ...
  const handleEndStream = async () => {
    setLoading(true);
    try {
      call.stopLive({ continue_recording: false });
      call.endCall();
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      const ch = client.channel("livestream", webinar.id, { name: webinar.title });
      await ch.watch();
          ...
          <div className="p-2 border-t border-border flex items-center justify-between py-2">
            ...
            {isHost && (
              <div className="flex items-center space-x-1">
                <Button
                  onClick={async () => {
                    await channel.sendEvent({ type: "start_live" });
                  }}
                  variant="outline"
                  className="mr-2"
                >
                ...
                <Button onClick={() => setObsDialogOpen(true)} variant="outline" className="mr-2">
                  Get OBS Creds
                </Button>
